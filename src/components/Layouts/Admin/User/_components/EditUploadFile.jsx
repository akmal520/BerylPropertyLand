...
const EditUploadFile = (props) => {
    const { setNewImages, propertyId } = props;
    ...
    const handleFileUpload = (event) => {
        ...
        const totalImages =
            imagePreview.length + existingImages.length + files.length;
        if (totalImages <= 5) {
    ...
    const handleRemoveExistingImage = async (imageId, imageUrl) => {
        const fileName = imageUrl.split('/').pop();
        const { data: storageData, error: storageError } =
            await supabase.storage.from('property_images').remove([fileName]);
    ...
    const handleRemoveImage = (index, isExisting = false, imageId = null) => {
        if (isExisting) {
            const imageToRemove = existingImages[index];
            handleRemoveExistingImage(imageId, imageToRemove.image_url);
        } else {
                ...
                {existingImages.map((image, index) => (
                    <div key={index} className="relative">
                        <button
                            ...
                            onClick={() => {
                                handleRemoveImage(index, true, image.id);
                            }}
                        >
                            <X className="w-4 h-4" />
                        </button>
